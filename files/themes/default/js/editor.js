...
function smc_Editor(oOptions)
{
	...
	this.iArrayPosition = bof_editorArray.length;
...
smc_Editor.prototype.init = function()
{
	...
	if (document.getElementById(this.opt.sUniqueId + '_resizer') && (!is_opera || is_opera95up) && !(is_chrome && !this.bRichTextEnabled))
	{
		...
		window.bof_oCurrentResizeEditor = null;
	...
	else
	{
		...
		if (this.bRichTextEnabled)
		{
			...
			else
			{
				...
				this.bof_execCommand('inserthtml', false, typeof(iMoveCursorBack) == 'undefined' ? sText : sText.substr(0, sText.length - iMoveCursorBack));
			}
		}
		else
		{
...
smc_Editor.prototype.bof_execCommand = function(sCommand, bUi, sValue)
{
	...
	else
	{
		...
		else
		{
			...
			if (oButtonProperties.sCode in this.oSimpleExec)
				this.bof_execCommand(this.oSimpleExec[oButtonProperties.sCode], false, null);
...
smc_Editor.prototype.handleSelectChange = function (oSelectProperties)
{
	...
	if (oSelectProperties.sName == 'sel_face')
	{
		...
		{
			if (is_webkit)
				this.bof_execCommand('styleWithCSS', false, true);
			this.bof_execCommand('fontname', false, sValue);
		}
	}
	...
	else if (oSelectProperties.sName == 'sel_size')
	{
		...
		if (!this.bRichTextEnabled)
		{
			...
			this.bof_execCommand('fontsize', false, sValue);
	}
	// Or color even?
	else if (oSelectProperties.sName == 'sel_color')
	{
		...
		if (!this.bRichTextEnabled)
		{
			...
			this.bof_execCommand('forecolor', false, sValue);
	}
...
smc_Editor.prototype.insertLink = function(sType)
{
	...
	if (sType != 'email' && sType != 'ftp' && is_ie)
		this.bof_execCommand('createlink', true, 'http://');
	...
	else
	{
		...
		{
			this.bof_execCommand('unlink');
			this.bof_execCommand('createlink', false, sText);
		}
		else
...
smc_Editor.prototype.insertImage = function(sSrc)
{
	...
	this.bof_execCommand('insertimage', false, sSrc);
}
...
smc_Editor.prototype.removeFormatting = function()
{
	...
	if (this.bRichTextEnabled)
	{
		this.bof_execCommand('removeformat');
		this.bof_execCommand('unlink');
	}
...
smc_Editor.prototype.requestParsedMessage = function(bView)
{
	...
	this.tmpMethod(bof_prepareScriptUrl(bof_scripturl) + 'action=jseditor;view=' + (bView ? 1 : 0) + ';' + this.opt.sSessionVar + '=' + this.opt.sSessionId + ';xml', 'message=' + sText, this.onToggleDataReceived);
	delete tmpMethod;
}
...
smc_Editor.prototype.spellCheckStart = function()
{
	...
	if (this.bRichTextEnabled)
	{
		...
		this.tmpMethod = sendXMLDocument;
		this.tmpMethod(bof_prepareScriptUrl(bof_scripturl) + 'action=jseditor;view=0;' + this.opt.sSessionVar + '=' + this.opt.sSessionId + ';xml', 'message=' + sText, this.onSpellCheckDataReceived);
		delete tmpMethod;
	}
...
smc_Editor.prototype.spellCheckEnd = function()
{
	...
	if (this.bRichTextEnabled)
	{
		...
		this.tmpMethod = sendXMLDocument;
		this.tmpMethod(bof_prepareScriptUrl(bof_scripturl) + 'action=jseditor;view=1;' + this.opt.sSessionVar + '=' + this.opt.sSessionId + ';xml', 'message=' + sText, bof_editorArray[this.iArrayPosition].onSpellCheckCompleteDataReceived);
		delete tmpMethod;
	}
	else
		this.setFocus();
...
smc_Editor.prototype.startResize = function(oEvent)
{
	...
	if (!oEvent || window.bof_oCurrentResizeEditor != null)
		return true;

	window.bof_oCurrentResizeEditor = this.iArrayPosition;

	var aCurCoordinates = bof_mousePose(oEvent);
...
smc_Editor.prototype.resizeOverIframe = function(oEvent)
{
	...
	var newCords = bof_mousePose(oEvent);
...
smc_Editor.prototype.endResize = function (oEvent)
{
	...
	if (window.bof_oCurrentResizeEditor == null)
		return true;
