function bof_StatsCenter(oOptions)
{
	this.opt = oOptions;
...
bof_StatsCenter.prototype.init = function ()
{
				...
				aSwapImages: [
					{
						...
						srcExpanded: bof_images_url + '/' + this.opt.sYearImageExpanded,
						altExpanded: '-',
						srcCollapsed: bof_images_url + '/' + this.opt.sYearImageCollapsed,
						altCollapsed: '+'
					}
				],
				aSwapLinks: [
				...
				aSwapImages: [
					{
						...
						srcExpanded: bof_images_url + '/' + this.opt.sMonthImageExpanded,
						altExpanded: '-',
						srcCollapsed: bof_images_url + '/' + this.opt.sMonthImageCollapsed,
						altCollapsed: '+'
					}
				],
				aSwapLinks: [
...
bof_StatsCenter.prototype.onBeforeCollapseYear = function (oToggle)
{
...
bof_StatsCenter.prototype.onBeforeCollapseMonth = function (oToggle)
{
	if (!oToggle.bCollapsed)
	{
		// Tell SMF that it the state has changed.
		getXMLDocument(bof_prepareScriptUrl(bof_scripturl) + 'action=stats;collapse=' + oToggle.opt.sMonthId + ';xml');
...
bof_StatsCenter.prototype.onBeforeExpandMonth = function (oToggle)
{
	...
	if (oToggle.opt.aSwappableContainers.length == 0)
	{
		...
		this.oXmlRequestHandle = this.tmpMethod(bof_prepareScriptUrl(bof_scripturl) + 'action=stats;expand=' + oToggle.opt.sMonthId + ';xml', this.onDocReceived);
	...
	// Silently let SMF know this one is expanded.
		...
		getXMLDocument(bof_prepareScriptUrl(bof_scripturl) + 'action=stats;expand=' + oToggle.opt.sMonthId + ';xml');
}

bof_StatsCenter.prototype.onDocReceived = function (oXMLDoc)
{
